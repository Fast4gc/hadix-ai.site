  ...
  cat > "$BACKEND_DIR/api/package.json" <<'PKG'
{
  ...
  "dependencies": { "express": "^5.1.0", "express-rate-limit": "^8.1.0", "helmet": "^8.1.0" }
}
PKG
...
.msg-meta { color: var(--hadix-muted); font-size: .7rem; margin-top: .4rem; }
.chat-scroll { max-height: calc(100vh - 300px); overflow-y: auto; }
...
<div class="container" style="max-width: 720px;">
  <div class="text-center py-4">
    ...
    <p class="muted small mb-0">Converse com as IAs do Ollama e gere imagens.</p>
  </div>
  ...
  <div id="viewChat" class="d-none">
    ...
    <div class="btn-group btn-group-sm mb-2 w-100" role="group" aria-label="Modo">
      <button id="modeChatBtn" type="button" class="btn btn-lime"><i class="bi bi-chat-dots"></i> Conversar</button>
      <button id="modeImageBtn" type="button" class="btn btn-ghost"><i class="bi bi-image"></i> Imagem</button>
    </div>
    <div id="chatPane" class="card card-hadix p-3">
    ...
    <div id="imagePane" class="d-none">
      <div class="card card-hadix p-3">
        <label class="form-label" for="imgPrompt">Prompt da imagem</label>
        <textarea id="imgPrompt" class="form-control mb-2" rows="3" placeholder="Descreva a imagem... ex.: raposa geométrica verde-lima em fundo escuro, estilo minimalista"></textarea>
        <div class="d-flex gap-2 mb-2">
          <select id="imgSize" class="form-select" style="max-width: 200px" aria-label="Tamanho">
            <option value="square">Quadrada 768</option>
            <option value="landscape">Paisagem 1024×576</option>
            <option value="portrait">Retrato 576×1024</option>
          </select>
          <button id="imgBtn" class="btn btn-lime" type="button"><i class="bi bi-magic"></i> Gerar</button>
        </div>
        <div id="imgResult" class="text-center py-3 muted small">A imagem gerada aparece aqui.</div>
      </div>
    </div>
  </div>
</div>
...
(function () {
  const $ = id => document.getElementById(id);
  const SYS = 'Você é o Hadix AI, assistente técnico direto e prestativo. Responda em português do Brasil. Ao escrever código, use blocos com três backticks e o nome da linguagem.';
  ...
  function setMode(mode) {
    ...
    $('chatPane').classList.toggle('d-none', !chat);
    $('imagePane').classList.toggle('d-none', chat);
    $('modeChatBtn').className = 'btn ' + (chat ? 'btn-lime' : 'btn-ghost');
    $('modeImageBtn').className = 'btn ' + (chat ? 'btn-ghost' : 'btn-lime');
    if (chat) $('chatInput').focus(); else $('imgPrompt').focus();
  }
  ...
  function requestMessages() {
    const recent = [];
    let used = SYS.length;
    for (let i = history.length - 1; i >= 0 && recent.length < HISTORY_MAX; i--) {
  ...
  async function sendChat(text) {
    ...
    try {
      ...
      const content = data.message?.content || '(resposta vazia)';
      history.push({ role: 'assistant', content });
      aiDiv.textContent = content;
      const meta = document.createElement('div');
      meta.className = 'msg-meta';
      meta.textContent = ((Date.now() - t0) / 1000).toFixed(1) + 's · ' + ($('chatModel').textContent);
      aiDiv.appendChild(meta);
    } catch (err) {
      if (err && err.name === 'AbortError') {
        aiDiv.textContent = '(enviado interrompido)';
      } else {
  ...
  async function generateImage() {
    ...
    box.innerHTML = '<div class="muted small">Isso pode levar de 10 a 60 segundos...</div>';
    const t0 = Date.now();
    try {
      const res = await fetch(cfg().base.replace(/\/$/, '') + '/api/image', {
        method: 'POST',
      ...
      if (!res.ok) {
        ...
        if (ct.includes('json')) { const j = await res.json().catch(() => ({})); msg = j.message || j.error || msg; }
        throw new Error(msg);
      }
      ...
      img.src = url; img.className = 'img-fluid rounded'; img.alt = prompt;
      const row = document.createElement('div');
      row.className = 'd-flex gap-2 mt-2 align-items-center justify-content-center';
      const a = document.createElement('a');
      a.className = 'btn btn-sm btn-ghost'; a.href = url;
      a.download = 'hadix-' + Date.now() + '.jpg';
      a.innerHTML = '<i class="bi bi-download"></i> Baixar';
      const t = document.createElement('span');
      t.className = 'muted small';
      t.textContent = secs + 's · ' + $('imgSize').selectedOptions[0].textContent;
      row.append(a, t);
      box.append(img, row);
    } catch (err) {
      box.innerHTML = '';
      const e = document.createElement('div');
      e.className = 'alert alert-danger py-2 small mb-0';
      e.textContent = 'Erro ao gerar: ' + (err.message || err);
      box.appendChild(e);
    } finally {
      btn.disabled = false;
      btn.innerHTML = '<i class="bi bi-magic"></i> Gerar';
    }
  }
