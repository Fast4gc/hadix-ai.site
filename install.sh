...
<meta name="theme-color" content="#08090a">
<link rel="icon" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 32 32'%3E%3Crect width='32' height='32' rx='7' fill='%2308090a'/%3E%3Cpath d='M16 5v22M5 16h22M8 8l16 16M8 24L24 8' stroke='%23d6ecc1' stroke-width='2'/%3E%3C/svg%3E">
...
async function send(text) {
 ...
 const progress=setInterval(()=>{
  if(inflight!==request||generation!==epoch)return;
  const seconds=Math.floor((performance.now()-started)/1000);
  $('chatNotice').textContent='Aguardando a IA · '+seconds+' s'+(seconds>=30?' · O servidor ainda não devolveu a resposta. Você pode parar a geração.':' · A resposta aparecerá quando a geração terminar.');
 },1000);
 ...
 try {
  ...
  clearTimeout(timeout);clearInterval(progress);if(inflight===request)inflight=null;
