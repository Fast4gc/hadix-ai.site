...
(async () => {
  ...
  const site = http.createServer((req, res) => {
    ...
    const file = req.url === '/chat.html' ? 'chat.html' : req.url === '/app/' ? 'app/index.html' : 'dashboard.html';
  ...
  try {
    ...
    await page.getByRole('button',{name:'Conectar API'}).click();
    ...
    const messagesBefore=await page.locator('.message').count();await page.reload();await page.waitForFunction(()=>document.querySelector('#sessionStatus').textContent==='Pronta');assert.equal(await page.locator('.message').count(),messagesBefore);
    ...
    const limited=await bounded.newPage();await limited.goto(siteURL);await limited.waitForFunction(()=>document.querySelector('#sessionStatus').textContent==='Pronta');await limited.locator('#chatInput').fill('Continue');await limited.locator('#sendBtn').click();await limited.waitForFunction(()=>document.querySelector('#stopBtn').hidden);assert.ok(calls.at(-1).messages.length<=24);assert.ok(calls.at(-1).messages.reduce((n,m)=>n+m.content.length,0)<=16000);assert.equal(await limited.locator('.message').count(),32);
    await limited.locator('#exportFormat').selectOption('svg');const zipWait=limited.waitForEvent('download');await limited.locator('#exportBtn').click();const zip=await zipWait;assert.match(zip.suggestedFilename(),/\.zip$/);await zip.saveAs(path.join(results,'export-long.zip'));report.push({test:'context-and-pagination',max24:true,max16000:true,historyPreserved:true,zip:true});await bounded.close();
    ...
    await hostPage.locator('#exportBtn').click();await hostPage.waitForFunction(()=>document.querySelector('#exportNotice').textContent==='Imagem salva.');assert.ok(fs.readFileSync(savedExport).subarray(0,8).equals(Buffer.from([137,80,78,71,13,10,26,10])));
